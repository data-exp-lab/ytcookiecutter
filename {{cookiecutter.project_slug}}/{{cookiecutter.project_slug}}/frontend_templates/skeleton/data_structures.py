...
class {{ cookiecutter.frontend_name }}Grid(AMRGridPatch):
    _id_offset = 0
    ...
    def __repr__(self):
        return "{{ cookiecutter.frontend_name }}Grid_%04i (%s)" % (self.id, self.ActiveDimensions)


class {{ cookiecutter.frontend_name }}Hierarchy(GridIndex):
    grid = {{ cookiecutter.frontend_name }}Grid

    def __init__(self, ds, dataset_type="{{ cookiecutter.frontend_name|lower }}"):
...
class {{ cookiecutter.frontend_name }}Dataset(Dataset):
    _index_class = {{ cookiecutter.frontend_name }}Hierarchy
    _field_info_class = {{ cookiecutter.frontend_name }}FieldInfo

    def __init__(
        self,
        filename,
        dataset_type="{{ cookiecutter.frontend_name|lower }}",
        storage_filename=None,
        units_override=None,
        unit_system="cgs",
        default_species_fields=None,
    ):
        self.fluid_types += ("{{ cookiecutter.frontend_name|lower }}",)
        super().__init__(
            filename,
            dataset_type,
